...
- **Project media (Projects only):** a representative 16:9 thumbnail and a horizontally scrolling strip of app screenshots (`public/projects/<slug>/`, webp, `next/image`) sit between the links row and the itemized sections, so a visitor can judge the product without leaving the site. Sections follow the README arc — 문제 의식 → 해결 → 사용 기술 → 담당한 기능 — written as sentences ("~를 했습니다"), not fragments. The Projects list stays text-only: title, a one-line summary set smaller than the title, and stack chips.
